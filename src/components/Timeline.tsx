...
const Timeline = () => {
  ...
  const events: TimelineEvent[] = [
    ...
    {
      time: '19:00 hs',
      title: 'Ceremonia Civil y Festejo',
      location: 'Jockey Club, Salta',
          ...
          {events.map((event, index) => (
            <div key={index} className="relative flex items-start gap-6 pl-4">
              ...
              <div className="flex-1 bg-card/40 backdrop-blur-sm rounded-xl p-5 border border-gold-light/20 shadow-sm">
                <span className="text-s font-display text-xl text-foreground mb-2 mt-1">{event.time}</span>
                <h3 className="text-xl text-gold font-display italic">{event.title}</h3>
                <button
